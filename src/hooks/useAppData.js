...
export function useAppData(session, navigate, pathname, authIsLoading) {
  ...
  return useMemo(() => ({
    ...
    handleDeleteAccount: async (id) => { const err = await baseData.handleDeleteAccount(session, id); if (!err) await transactionsData.fetchTransactions(userIdRef.current); return err; },
